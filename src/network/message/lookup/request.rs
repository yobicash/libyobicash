...
//! The `request` module provides the Yobicash network lookup request message type and methods.
...
/// The request for looking up a resource.
...
pub struct LookupRequest {
    ...
    /// The maximum size of the `LookupRequest` message.
    pub max_size: u32,
    /// The resource type.
...
impl LookupRequest {
    ...
    pub fn new(session: &Session,
               ...
               resource_id: Digest) -> Result<LookupRequest> {
        session.validate()?;
...
impl<'a> Serialize<'a> for LookupRequest {
