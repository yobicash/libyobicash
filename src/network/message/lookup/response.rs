...
//! The `response` module provides the Yobicash network lookup response message type and methods.
...
/// The successful response to a lookup request message.
...
pub struct LookupResponse {
    ...
    /// The maximum size of the `LookupResponse` message.
    pub max_size: u32,
    /// The resource type.
...
impl LookupResponse {
    ...
    pub fn new(session: &Session,
               ...
               found: bool) -> Result<LookupResponse> {
        session.validate()?;
        ...
        if lookup_response.to_bytes()?.len() as u32 > max_size {
...
impl<'a> Serialize<'a> for LookupResponse {
    ...
    fn from_json(s: &str) -> Result<Self> {
        ...
        let found_value = obj["found"].clone();
        let found: bool = json::from_value(found_value)?;
    ...
    fn from_bytes(b: &[u8]) -> Result<Self> {
        let lookup_response = messagepack::from_slice(b)?;
