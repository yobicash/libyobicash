...
//! The `response` module provides the Yobicash network list response message type and methods.
...
/// The successful response to a list request message.
...
impl<'a> Serialize<'a> for ListResponse {
    ...
    fn from_bytes(b: &[u8]) -> Result<Self> {
        let list_response = messagepack::from_slice(b)?;
