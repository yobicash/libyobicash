use chrono::Duration;
use libyobicash::models::tx::regular::*;
...
#[test]
fn new_tx_succ() {
    let res = RegularTx::new();
    assert!(res.is_ok())
}

#[test]
fn set_time_succ() {
    let mut tx = RegularTx::new().unwrap();
...
fn unique_txs_succ() {
    let len = 10;
    let mut txs: Vec<RegularTx> = Vec::new();
    for i in 0..len {
        let mut tx = RegularTx::new().unwrap();
    ...
    let res = check_unique_regtxs(&txs);
    assert!(res.is_ok())
}
...
fn unique_txs_fail() {
    let tx = RegularTx::new().unwrap();
    let len = 10;
    let txs: Vec<RegularTx> = repeat(tx).take(len).collect();
    let res = check_unique_regtxs(&txs);
    assert!(res.is_err())
}
...
fn get_outpoint_succ() {
    ...
    tx.check_signatures().unwrap();
    tx.finalize().unwrap();
    let output = tx.get_output(0).unwrap();
