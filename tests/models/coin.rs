...
fn coin_new_succ() {
    ...
    let res = Coin::new(&output, instance);
    assert!(res.is_ok())
}
...
fn coin_new_fail() {
    ...
    let res = Coin::new(&output, instance_b);
    assert!(res.is_err())
}
...
fn coin_validate_fail() {
    ...
    let mut coin = Coin::new(&output, instance).unwrap();
...
fn coin_to_json_succ() {
    ...
    let coin_a = Coin::new(&output, instance).unwrap();
